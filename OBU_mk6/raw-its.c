...
// Local Variables
//------------------------------------------------------------------------------
struct RAWexternedata pudpclient;
                               ...
                               void *pPriv)
{
  ...
  if (Event == QSMSG_EXT_RX_ITSFL_MSG)
  {
	...
	printf("recive lon through wlanp: %f\n",pudpclient.lon);
        ...
        if (resudpclient)
  	{
    		d_error(D_ERR, 0, "UDP client failed\n");
  	}
      }
    }
...
int udpclient()
{
  ...
  // struct udpsendbuf message[2]={pudpclient};
     
     //json;
		cJSON *json = cJSON_CreateObject();
        
		cJSON_AddNumberToObject(json,"lat",pudpclient.lat);
		cJSON_AddNumberToObject(json,"lon",pudpclient.lon);
        ...
        //send the message
        if (sendto(s, json_data, strlen(json_data) , 0 , (struct sockaddr *) &si_other, slen)==-1)
        {
            die("sendto()");
        }
